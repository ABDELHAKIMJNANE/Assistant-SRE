"""Tests unitaires du Sanitizer."""

from app.services.sanitizer import sanitize_text, sanitize_logs


class TestSanitizeText:
    """Tests de nettoyage des données sensibles."""
    ...
    def test_ip_masking(self):
        ...
        assert "10.0.1.50" not in cleaned
    ...
    def test_normal_text_unchanged(self):
        ...
        assert "150ms" in cleaned


class TestSanitizeLogs:
    """Tests de troncature et nettoyage de listes de logs."""

    def test_truncation(self):
        logs = [f"Log line {i}" for i in range(100)]
        result = sanitize_logs(logs, max_lines=50)
        assert len(result) == 50
        # Doit garder les 50 dernières lignes
